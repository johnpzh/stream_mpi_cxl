...
int is_fam_pointer(rapid_handle rapid, double* ptr);
...
main()
{
  ...
  k = rapid_posix_memalign(rapid, (void **)&a, array_alignment, array_bytes);
  if (k != 0) {
    printf("Rank %d: Allocation of array a failed, return code is %d\n",myrank,k);
    MPI_Abort(MPI_COMM_WORLD, 2);
    exit(1);
  }
  if (is_fam_pointer(rapid, a)) {
    printf("Rank %d: Array a is allocated on FAM\n", myrank);
  } else {
    printf("Rank %d: Array a is not allocated on FAM. Something is wrong. Exiting...\n",myrank);
    MPI_Abort(MPI_COMM_WORLD, 2);
    exit(1);
  }

  // k = posix_memalign((void **)&b, array_alignment, array_bytes);
  k = rapid_posix_memalign(rapid, (void **)&b, array_alignment, array_bytes);
  if (k != 0) {
    printf("Rank %d: Allocation of array b failed, return code is %d\n",myrank,k);
    MPI_Abort(MPI_COMM_WORLD, 2);
    exit(1);
  }
  if (is_fam_pointer(rapid, b)) {
    printf("Rank %d: Array b is allocated on FAM\n", myrank);
  } else {
    printf("Rank %d: Array b is not allocated on FAM. Something is wrong. Exiting...\n",myrank);
    MPI_Abort(MPI_COMM_WORLD, 2);
    exit(1);
  }

  // k = posix_memalign((void **)&c, array_alignment, array_bytes);
  k = rapid_posix_memalign(rapid, (void **)&c, array_alignment, array_bytes);
  if (k != 0) {
    printf("Rank %d: Allocation of array c failed, return code is %d\n",myrank,k);
    MPI_Abort(MPI_COMM_WORLD, 2);
    exit(1);
  }
  if (is_fam_pointer(rapid, c)) {
    printf("Rank %d: Array c is allocated on FAM\n", myrank);
  } else {
    printf("Rank %d: Array c is not allocated on FAM. Something is wrong. Exiting...\n",myrank);
    MPI_Abort(MPI_COMM_WORLD, 2);
    exit(1);
  }
 ...
 */
int is_fam_pointer(rapid_handle rapid, STREAM_TYPE* ptr){
  void* base = NULL;
  void* last = NULL;
